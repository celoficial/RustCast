...
fn build_alive_messages(location: &str, udn: &str, multicast_host: &str) -> Vec<String> {
    let entries = [
        ...
        (udn,                                            udn.to_string()),
        ("urn:schemas-upnp-org:device:MediaServer:1",   format!("{}::urn:schemas-upnp-org:device:MediaServer:1", udn)),
    ];
    entries.iter().map(|(nt, usn)| format!(
        "NOTIFY * HTTP/1.1\r\n\
         HOST: {}\r\n\
...
pub fn start_ssdp_advertiser(config: Config) -> tokio::task::JoinHandle<()> {
    tokio::spawn(async move {
        let location = format!("http://{}:{}/description.xml", config.http_address, config.http_port);
        let target = format!("{}:{}", config.multicast_address, config.multicast_port);
        ...
        println!("SSDP advertiser started — announcing as '{}'", config.friendly_name);
